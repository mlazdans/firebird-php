...
if test "$PHP_FIREBIRD" != "no"; then
  ...
  PHP_NEW_EXTENSION(firebird, [firebird.c database.c connection.c transaction.c statement.c blob.c error.c args_connect.c args_create.c var_info.c event.c], $ext_shared,, -DZEND_ENABLE_STATIC_TSRMLS_CACHE=1,[cxx])
